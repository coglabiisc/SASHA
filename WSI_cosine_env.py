...
#             high_cosine_indices = (cosine_vector >= self.cosine_threshold).nonzero()[:, 0]
...
#                 list(set(high_cosine_indices.tolist()).intersection(valid_indices.tolist())), device= device)
#             v_at = self.hr_features[action].to(device)
#             # Just updating the patch if no other patch have same cosine similarity
#             if high_cosine_indices.shape[0] == 0:
#                 self.current_state[0][action] = v_at
...
#                 reward = self.reward[-1] - self.reward[-2]
#             else:
#                 # reward = - 2 * (self.current_time_step / (self.max_time_steps * (self.max_time_steps - 1))) * loss.item()
#                 reward = - loss.item()
...
#         # return torch.nonzero(self.visited_patches == 1, as_tuple=True)[0].tolist()
#         return self.visited_patches.tolist()
...
#                 final_state = self.current_state.clone()
#                 self.reset()
#                 return final_state, self.reward[-1], done
#             else:
#                 return self.current_state, self.reward[-1], done
#
#         if self.current_time_step < self.max_time_steps:
#             self.current_time_step += 1
#             self.visited_patches[action] = 1
