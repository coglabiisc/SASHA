...
class MIL_fc(nn.Module):
    ...
    def forward(self, h, return_features=False, testing=True):
        # h.shape = (K, 1024)
        # self.top_k = 1
        if return_features:
        ...
        y_probs = F.softmax(logits, dim = 1) # K x 2
        top_instance_idx = torch.topk(y_probs[:, 1], self.top_k, dim=0)[1].view(1,) # (top_k), select highest probability of label 1
        top_instance = torch.index_select(logits, dim=0, index=top_instance_idx) # 1 X 2
