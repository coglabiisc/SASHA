...
class CLAM_SB(nn.Module):
    ...
    def __init__(self, gate = True, size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        ...
        if gate:
            ...
            attention_net = Attn_Net(L = size[0], D = size[1], dropout = dropout, n_classes = 1)
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False, train_epoch=-1,
        ...
        A = F.softmax(A / self.temp, dim=1)  # softmax over N
        if instance_eval:
            ...
            inst_labels = F.one_hot(label, num_classes=self.n_classes).squeeze() # Converting label to one hot representation
